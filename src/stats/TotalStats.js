class TotalStats {
  ...
  setTotalHits = (totalHits) => {
    if (totalHits !== typeof number) totalHits = parseInt(totalHits);

    this.#totalHits = totalHits;
  };

  incTotalHits = (amount) => {
    if (amount !== typeof number) amount = parseInt(amount);

    this.#totalHits = this.#totalHits + amount;
  };

  setTotalMisses = (totalMisses) => {
    if (totalMisses !== typeof number) totalMisses = parseInt(totalMisses);
   ...
   */
  getTotalAccuracy = () => {
    if (this.#totalHits === 0 && this.#totalMisses === 0) return 0;
